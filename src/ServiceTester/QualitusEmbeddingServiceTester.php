...
/**
 * Validates Qualitus AI embedding proxy token via minimal embedding request.
 */
class QualitusEmbeddingServiceTester implements IAiServiceTester {

	public static function getType(): string {
		return 'qualitusembedding';
	}

	public function test(array $config): array {
		$endpoint = $config['endpoint'] ?? 'https://qki-proto1.qualitus.net/base3.php?name=aiembeddingproxy';
		$token = $config['apikey'] ?? '';
		...
		if ($code >= 200 && $code < 300) {
			return [
				'ok' => true,
				'apikey_valid' => true,
				'message' => 'Qualitus embedding proxy OK'
			];
		}
